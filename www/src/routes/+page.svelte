...
<script>
  ...
  $: if(scene) {
  	...
  	scene.setSignal(Array(samples).fill(amplitude).map((a,i) => [a, (i/samples-0.5)]).map(([ampl,t]) => [ampl*shapes[shape](timeStretch*t-timeShift/samples)* Math.cos(Math.PI*2*(freq*2*t+phase/360)), ampl*shapes[shape](timeStretch*t-timeShift/samples)*Math.sin(Math.PI*2*(freq*2*t+phase/360))]))
  	
  	scene.setSpectrum(Array(samples).fill(amplitude).map((a,i) => [a, ((i-freq)/samples-0.5)]).map(([ampl, i]) => [ampl*shapes[transformPairs[shape]](i&&i/timeStretch)*Math.cos(Math.PI*2*(phase/360-2*i*timeShift)),ampl*shapes[transformPairs[shape]](i&&i/timeStretch)*Math.sin(Math.PI*2*(phase/360-2*i*timeShift))]))
  }
...
<div class="container">
	...
	<div class="controls">
		<fieldset>
			...
			<label>Shape: {transformPairs[shape]} 
				...
				<label>Time Shift: <output>{intFormat.format(timeShift)}</output>
					<input type="range" min="-15" max="15" step="1" bind:value={timeShift} name="">
				</label>
				<label>Time Stretch: <output>{intFormat.format(timeStretch)}</output>
					<input type="range" min="0" max="15" step="0.125" bind:value={timeStretch} name="">
				</label>
		</label>
			...
			<label>Frequency:  <output>{intFormat.format(freq)}</output> 
				...
				<input type="range" min="-15" max="15" step="1" bind:value={freq} name=""></label>
