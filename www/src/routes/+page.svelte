<script>
    ...
    const decimalFormatSigned = new Intl.NumberFormat("en-IN", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    ...
    let demoMode = false;
    let syncRot = false;
    let timeDomain = new Float32Array(
        wasm.memory.buffer,
        signal.get_time(),
        2 * signal.get_len(),
    );
    let freqDomain = new Float32Array(
        wasm.memory.buffer,
        signal.get_freq(),
        2 * signal.get_len(),
    );
    let fracDomain = new Float32Array(
        wasm.memory.buffer,
        signal.get_frac(),
        2 * signal.get_len(),
    );
    ...
    function recordTouch(evt) {
        if (!recField) return;
        let x =
            (evt.touches[0].pageX - recField.offsetLeft) / recField.offsetWidth;
        let y =
            (evt.touches[0].pageY - recField.offsetTop) / recField.offsetHeight;
    ...
    function recordDo() {
        ...
        if ((rbs & 4) == 4) {
            ...
            customRecording[2 * r + 1] =
                Math.sign(rx) * Math.sqrt(Math.abs(rx));
        } else {
    ...
    const shapes = {
        ...
        phimod: (x) => (((((x / 2) * 1.618033988749895) % 1) + 1) % 1) - 0.5,
    };
        ...
        if (shape !== "") {
            for (let i = 0; i < samples; i++) {
                ...
                const mag =
                    ...
                    shapes[shape](
                        timeStetchExp * t * 16,
                        samples / (timeStetchExp * 16),
                    );
                const phi = Math.PI * 2 * (freq * 2 * t + phase / 360);

                timeDomain[
                    (2 * i - 2 * timeShift + samples * 2) % (samples * 2)
                ] = mag * Math.cos(phi);
                timeDomain[
                    (2 * i + 1 - 2 * timeShift + samples * 2) % (samples * 2)
                ] = mag * Math.sin(phi);
            }
        } else {
            for (let i = 0; i < samples; i++) {
    ...
    $: {
        if (timeDomain.byteLength === 0) {
            ...
            freqDomain = new Float32Array(
                wasm.memory.buffer,
                signal.get_freq(),
                2 * signal.get_len(),
            );
            fracDomain = new Float32Array(
                wasm.memory.buffer,
                signal.get_frac(),
                2 * signal.get_len(),
            );
    ...
    $: paintPath = customRecording.reduce(
        (acc, n, i) =>
            acc + (i % 2 == 0 ? getPathA(n, 0.03) : getPathB(n, 0.03)),
        "",
    );
    $: paintPathEmpty = !Array.prototype.some.call(
        customRecording,
        (a) => a != 0,
    );
...
<svelte:window
    on:mousemove={record}
    on:touchmove={recordTouch}
    on:mouseup={recordStop}
    on:touchend={recordStop}
    on:touchcancel={recordStop}
/>
        ...
        <div class="controls-inner">
            <fieldset>
                <legend style="white-space: nowrap;"
                    >Fractional Discrete <br />Fourier Transform</legend
                >
                ...
                {#if shape}
                    <div>
                        <br />
                        <label for="control_amplitude"
                            ><span
                                style:display="flex"
                                style:gap="0.2em"
                                style:white-space="nowrap"
                                >Amplitude: <output
                                    >{decimalFormat.format(amplitude)}</output
                                ></span
                            >
                            <input
                                list={snap ? "ampl-list" : null}
                                type="range"
                                min="0"
                                max="2"
                                step="0.01"
                                bind:value={amplitude}
                                id="control_amplitude"
                            /></label
                        ><br />
                        <hr />
                        <label for="control_timeStretch"
                            ><span
                                style:display="flex"
                                style:gap="0.2em"
                                style:white-space="nowrap"
                            ...
                            <input
                                ...
                                min="-5"
                                max="5"
                                step={true || snap ? 1 : 0.01}
                                bind:value={timeStretch}
                                id="control_timeStretch"
                            />
                        </label>
                        <label for="control_timeShift"
                            ><span
                                style:display="flex"
                                style:gap="0.2em"
                                style:white-space="nowrap"
                                >Time Shift: <output
                                    >{intFormat.format(timeShift)}</output
                                ></span
                            >
                            <input
                                list={snap ? "freq-list" : null}
                                type="range"
                                min="-{(samples * 3) / 4}"
                                max={(samples * 3) / 4}
                                step="1"
                                bind:value={timeShift}
                                id="control_timeShift"
                            />
                        </label>

                        <hr />
                        <label for="control_phase"
                            ><span
                                style:display="flex"
                                style:gap="0.2em"
                                style:white-space="nowrap"
                                >Constant Phase: <output
                                    >{intFormat.format(phase)}°</output
                                >
                            </span>

                            <input
                                list={snap ? "phase-list" : null}
                                type="range"
                                min="-180"
                                max="180"
                                step="5"
                        ...
                        <label for="control_freq"
                            ><span
                                style:display="flex"
                                style:gap="0.2em"
                                style:white-space="nowrap"
                                >Linear Phase: <output
                                    >{intFormat.format(freq)}</output
                                >
                            </span>
                ...
                <label for="control_fraction">
                    <span
                        ...
                        style:gap="0.2em"
                        style:white-space="nowrap"
                        >Fractional DFT: <output
                            >{decimalFormatSigned.format(fraction)}</output
                        ></span
                    >
                    <input
                        disabled={syncRot}
                        list={snap ? "frac-list" : null}
                        type="range"
                        min="-4"
                        max="4"
                        step="0.01"
                ...
                <!-- <label for="control_cepstrum">
                    <span
                        ...
                        style:white-space="nowrap"
                        >Cepstrum: <output
                            >{decimalFormatSigned.format(cepstrum)}</output
                        ></span
                    >
                    <input
                        type="range"
                ...
                <div class="checkbox-list">
                    <label
                        ><input type="checkbox" bind:checked={circular} /> Cyclic
                        axis</label
                    >
                    <label
                        ><input type="checkbox" bind:checked={snap} /> Snap Controls</label
                    >
                </div>
                <hr />
                ...
                <small
                    >&sdot;<button
                        ...
                        on:keydown={() => (showInfo = true)}>Info</button
                    ></small
                >
                <br />
                <small
                    >&sdot;<a
                        class="info-button"
                        href="https://www.youtube.com/watch?v=Xe2Ob1gPqlg"
                        >Video Showcase</a
                    ></small
                >
                <br />
                <small
                    >&sdot;<a
                        ...
                        target="_blank">More educational tools</a
                    ></small
                >
    ...
    >
        ...
        >
            <button
                ...
                class="info-button"
                on:click={() => (showInfo = false)}
                on:keydown={() => (showInfo = false)}>Close</button
            >
            ...
            <label
                ><input type="checkbox" bind:checked={demoMode} /> Demo Mode (Reload
                to exit)</label
            >
        </div>
    </div>
