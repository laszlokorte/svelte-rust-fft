...
export const createScene = (el: HTMLCanvasElement, camFrame: HTMLElement) => {
  ...
  const labelGeoNew = new LineSegmentsGeometry(3, 1, [
    LineSegmentsGeometry.squareCapStart(-3, 3),
  ]);
  labelGeoNew.setPositions([0, 0, 0, 0, 0, 0]);

  const sideLabelGeoNew = new LineSegmentsGeometry(3, 1, [
    LineSegmentsGeometry.squareCapStart(0, 0),
  ]);
  sideLabelGeoNew.setPositions([0, 0, 0, 0, 0, 0]);
  ...
  const axisGeo = new LineSegmentsGeometry(3, 1, [
    LineSegmentsGeometry.roundCapStart,
    LineSegmentsGeometry.arrowCapEnd,
  ]);
  ...
  const rotations = [
    {
      rot: new THREE.Vector3(0, (0 * Math.PI) / 2, 0),
      color: 0x00ffff,
      shadow: true,
      showAxis: true,
      curve: curveGeo,
      sideLabelIndex: 0,
      xAxisLabel: 2,
      reflector: new THREE.Vector3(1, 1, -1),
    },
    {
      rot: new THREE.Vector3(0, (1 * Math.PI) / 2, 0),
      color: 0x00ff00,
      shadow: true,
      showAxis: true,
      curve: curveGeoAlt,
    ...
    {
      rot: new THREE.Vector3(0, (2 * Math.PI) / 2, 0),
      color: 0xff00ff,
      shadow: true,
      showAxis: true,
      curve: curveGeo,
    ...
    {
      rot: new THREE.Vector3(0, (3 * Math.PI) / 2, 0),
      color: 0xff0000,
      shadow: true,
      showAxis: true,
      curve: curveGeoAlt,
  ...
  for (let {
    ...
    const cubeMaterial = new THREE.MeshLambertMaterial({
      color: (color & color_mask_sub) | color_mask_add,
    });
    ...
    curveBarMaterial.endProjectionMul = new THREE.Vector3(
      reflector.x,
      reflector.y,
      reflector.z,
    );
    ...
    const curveShadows = new LineSegments(curve, [
      shadow1,
      shadow2,
      shadow3,
      shadow4,
    ...
    if (shadow) graph.add(curveShadows);
      ...
      if (newLength < 8.5 && currentFocus === null) {
        focusSide(
          indexOfSmallest(
            refRotations.map((x) =>
              x.angleTo(
                upVector.set(1, 1 / stretchHeight, 1).multiply(camera.position),
              ),
            ),
          ),
        );
  ...
  const refRotations = rotations.map((r) =>
    new THREE.Vector3(1, 0, 0).applyEuler(
      new THREE.Euler(r.rot.x, r.rot.y, r.rot.z),
    ),
  );
  ...
  const animate = () => {
    if (resizeRendererToDisplaySize(renderer)) {
      ...
      camera.setViewOffset(
        camFrame.offsetWidth,
        camFrame.offsetHeight,
        -camFrame.offsetLeft,
        -camFrame.offsetTop,
        window.innerWidth,
        window.innerHeight,
    ...
    if (renderer.xr.isPresenting && vrController !== null) {
      ...
      if (newLength < 8.5 * 0.05 && currentFocus === null) {
        ...
        newLength > 8.5 * 0.05 &&
        currentFocus === indexOfSmallest(transformedRefs)
      ) {
        focusSide(null);
      }
      ...
      for (let lblMat of labelMats) {
        lblMat.uniforms.linewidth.value =
          lblMat.baseWidth /
          Math.max(0.01, root.position.distanceTo(camera.position));
      }
      ...
      for (let bm of barMats) {
        bm.opacity =
          ...
          Math.min(
            ...
            Math.max(
              0,
              (camera.position.distanceTo(root.position) / 0.05 - 4) / 10,
            ),
          );
      }
    } else {
      ...
      for (let bm of barMats) {
        bm.opacity =
          ...
          Math.min(
            0.9,
            Math.max(0, (camera.position.distanceTo(root.position) - 4) / 10),
          );
      }
    }
    ...
    if (rotationSubscriber) {
      rotationSubscriber(
        ((((2 * Math.atan2(camera.position.x, camera.position.z)) / Math.PI +
          1) %
          4) +
          4) %
          4,
